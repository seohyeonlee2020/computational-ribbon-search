...
# This file was *autogenerated* from the file ../sage_script/combined_sage_script.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_32 = Integer(32); _sage_const_8 = Integer(8)
import snappy
...
print(f'filtered knots based on first 2 obstructions, {df.shape[_sage_const_0 ]} potential ribbon links')
...
num_known_ribbons = df[df['known_ribbon']].shape[_sage_const_0 ]
...
df = df.apply(factor_jones, axis=_sage_const_1 )

df = df[df['jones_factorable']]
print(f'checked for jones determinants, {df.shape[_sage_const_0 ]} potential ribbon links')
...
df = df.apply(check_products, axis = _sage_const_1 )
...
print(f'checked for mod 32, {df.shape[_sage_const_0 ]} potential ribbon links, {num_known_ribbons} known ribbons among them')
...
def check_mod(row):
	...
	return mod(det, _sage_const_8 ) == _sage_const_1 

df['mod8_condition'] = df.apply(check_mod, axis = _sage_const_1 )
df = df[df['mod8_condition']]
print(f'checked for mod 8 {df.shape[_sage_const_0 ]} potential ribbon links')
...
print(f'applied fox milnor condition, {df.shape[_sage_const_0 ]} potential ribbon links')

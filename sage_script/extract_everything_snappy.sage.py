...
# This file was *autogenerated* from the file /Users/seohyeonlee/knotinfo/sage_script/extract_everything_snappy.sage
...
for link in link_iterator:
    ...
    L = link.link()
    num_crossings = int(re.findall('L(\d+)[an]\d+', linkname)[_sage_const_0 ])
    linking_matrix = L.linking_matrix()
    link_dct[linkname]= {}

    link_dct[linkname]['num_crossings'] = num_crossings
    link_dct[linkname]['determinant'] = L.determinant()
    link_dct[linkname]['matrix_zero'] =  all(x == _sage_const_0  for row in linking_matrix for x in row)
    ...
    if linkname not in potential_ribbons:
        link_dct[linkname]['ribbon'] = _sage_const_0 
    else:
        if linkname in known_ribbons:
            link_dct[linkname]['ribbon'] = _sage_const_1 
        else:
            link_dct[linkname]['ribbon'] = _sage_const_2 
